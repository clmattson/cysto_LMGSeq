...
#above line sets $BASEDIR == . not /home/script/location.  courtney edits:
...
echo $BASEDIR/$INTERMED_DIR

#add basedir
## First Copy Barcodes only for relevant crosses 
cat $BASEDIR/$2 | grep -f - $BASEDIR/shared/barcodes5.fasta -A1 -w --no-group-separator > $BASEDIR/$INTERMED_DIR/barcodes5.fasta
#Repeat with reverse barcodes
cat $BASEDIR/$2 | grep -f - $BASEDIR/shared/barcodes5rev.fasta -A1 -w --no-group-separator > $BASEDIR/$INTERMED_DIR/barcodes5rev.fasta

#Now, copy entire files for 3' barcodes because we want all samples 
cp $BASEDIR/shared/barcodes3.fasta $BASEDIR/$INTERMED_DIR/barcodes3.fasta
cp $BASEDIR/shared/barcodes3rev.fasta $BASEDIR/$INTERMED_DIR/barcodes3rev.fasta
...
#Courtney: incorrect file name below, changed /GbBSeq_human_IAV/data/ to human_influenza_GbBSeq
#First, I'll note that the barcode files have been generated.
#Now will demultiplex cross and samples.
cutadapt -g file:barcodes5.fasta -G file:barcodes3rev.fasta -O 8 --no-indels --action=none --discard-untrimmed -o trimmed_{name1}_{name2}_$1_R1.fastq -p trimmed_{name1}_{name2}_$1_R2.fastq ~/human_influenza_GbBSeq/data/$1_R1.fastq ~/human_influenza_GbBSeq/data/$1_R2.fastq > cutadapt_demultiplexing_report.txt
